      ...
      if (pick_new_route) {
        ...
        while (true) {
          ...
          if (dest[rand_cust] == 1)
            ...
            // cannot be served
            if (!spare_veh) {
              non_served_cust.push_back(std::get<1>(current_route)[1]);
              pick_new_route = 1;
              break;
            }
        ...
        for (size_t i = 0; i < save.size(); ++i) {
          ...
          if (best_save.j == std::get<1>(current_route)[1] &&
            ...
            if (!enable_splits || splited[best_save.i] >= (size_t)max_splits) {
            ...
            if (!tw_violation && !site_dep_viol && !cap_viol) {
              ...
              if (was_splited) {
                ...
                auto splited_cap = std::get<2>(current_route) -
                                   (tmp_cap - split_demand[best_save.i]);
                ...
                if (std::get<3>(current_route).split_info[best_save.i] < 0.0 ||
            ...
            if (!enable_splits || splited[best_save.j] >= (size_t)max_splits) {
              common_veh.erase(
                  std::remove_if(common_veh.begin(), common_veh.end(),
                                 [&](const int &x) {
                                   return ((prob.vehicles[x].capacity +
            ...
            if (!tw_violation && !site_dep_viol && !cap_viol) {
              ...
              if (was_splited) {
                ...
                if (std::get<3>(current_route).split_info[best_save.j] < 0.0 ||
