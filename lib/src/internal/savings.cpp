...
std::vector<Solution> savings(const Problem &prob, size_t count) {
  ...
  for (size_t it = 0; it < count; ++it) {
    ...
    for (unsigned int i = 0; ++i; i < points)
    ...
    if (prob.customers[0].suitable_vehicles.empty()) {
    ...
    std::sort(save.begin(), save.end(),
              [](const S &a, const S &b) { return a.save_ij - b.save_ij > 0; });
    ...
    if (enable_splits && rand_split) {
      ...
      std::get<0>(current_route) = {*std::max_element(
          std::get<0>(current_route).begin(), std::get<0>(current_route).end(),
          [&](const int &a, const int &b) {
            return prob.vehicles[a].capacity < prob.vehicles[b].capacity;
          })};
      std::get<2>(current_route) =
          prob.vehicles[std::get<0>(current_route)[0]].capacity;
      std::get<3>(current_route).split_info[rand_cust] =
          std::get<2>(current_route) / prob.customers[rand_cust].demand;
      ...
      if (pick_new_route) {
        ...
        while (true) {
          ...
          if (dest[rand_cust] == 1)
            ...
            if (enable_splits && splited[rand_cust] < max_splits) {
              bool rand_split = 1;
              if (enable_splits)
                for (auto &a : std::get<0>(current_route))
                  if (prob.vehicles[a].capacity >= std::get<2>(current_route))
        ...
        for (size_t i = 0; i < save.size(); ++i) {
          ...
          if (best_save.j == std::get<1>(current_route)[1] &&
            ...
            if (!enable_splits || splited[best_save.i] > max_splits) {
            ...
            if (!tw_violation && !site_dep_viol && !cap_viol) {
              ...
              if (was_splited) {
                ...
                std::get<3>(current_route).split_info[best_save.i] =
                    std::get<2>(current_route) /
                    prob.customers[best_save.i].demand;

                split_demand[best_save.i] -= std::get<2>(current_route);
                ++splited[best_save.i];
                ...
                ++served;
                dest[best_save.i] = 1;
                std::get<3>(current_route).split_info[best_save.i] = 1.0;
                std::get<2>(current_route) = tmp_cap;
                std::get<0>(current_route) = common_veh;
              }

              current_customers.insert(current_customers.begin() + 1,
                                       best_save.i);
            ...
            if (!enable_splits || splited[best_save.j] > max_splits) {
              common_veh.erase(
                  std::remove_if(common_veh.begin(), common_veh.end(),
                                 [&](const int &x) {
                                   return ((prob.vehicles[x].capacity +
            ...
            // magic num for to much waiting
            if (times[best_save.j].start - times[best_save.j].current + offset >
                MAX_WAITING_TIME) {
              tw_violation = 1;
            }
            ...
            if (!tw_violation && !site_dep_viol && !cap_viol) {
              ...
              if (was_splited) {
                ...
                std::get<2>(current_route) =
                    prob.vehicles[std::get<0>(current_route)[0]].capacity;
                std::get<3>(current_route).split_info[best_save.j] =
                    std::get<2>(current_route) /
                    prob.customers[best_save.j].demand;

                split_demand[best_save.j] -= std::get<2>(current_route);
                ++splited[best_save.j];
                ...
                ++served;
                dest[best_save.j] = 1;
                std::get<3>(current_route).split_info[best_save.j] = 1.0;
                std::get<2>(current_route) = tmp_cap;
                std::get<0>(current_route) = common_veh;
              }
