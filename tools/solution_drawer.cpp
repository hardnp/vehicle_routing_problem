...
class SolutionDrawer {
  ...
  static constexpr auto RADIUS      = 3.1;
public:
  ...
  auto solution_parse(std::string const& filename) -> void {

    std::fstream fstream {filename, std::ios_base::in};
    std::string current_line;
    std::uint32_t route, vehicle, customer;
    std::getline(fstream, current_line); // Skipping the solution header.
    ...
    routes.resize(std::unique(std::begin(route_index), std::end(route_index)) - std::begin(route_index));
    for (std::size_t index = 0; index < route_index.size(); ++index) routes[route_index[index]].push_back(customers_id[index]);
    for (auto& route: routes) { route.insert(std::begin(route), 0); route.insert(std::end(route), 0); }

  }

  auto solomon_instance_parse(std::string const& filename) -> void {

    std::fstream fstream {filename, std::ios_base::in};
    std::string current_line;

    std::int64_t vehicle_amount, capacity;
    while (std::getline(fstream, current_line)) {
      std::istringstream iss (current_line);
    ...
    while (std::getline(fstream, current_line)) {
      std::istringstream iss (current_line);
      if (iss >> id >> x >> y) customers.emplace_back(CustomerData {id, x, y});
    }
  ...
  auto draw() -> void {
    ...
    auto map = [size, this](auto cx, auto cy) -> sf::Vector2<std::int64_t> {
        ...
        const std::int64_t x = nx * size.x * ZOOM_FACTOR + (1.0 - ZOOM_FACTOR) * size.x / 2;
        const std::int64_t y = ny * size.y * ZOOM_FACTOR + (1.0 - ZOOM_FACTOR) * size.y / 2;
        return {x, y};
    };
    ...
    for (auto const& route: routes) {
        ...
        for (auto const& customer: route) {
            sf::Vector2<std::int64_t> mapped = map(customers[customer].x, customers[customer].y);
            sf::CircleShape circle;
            circle.setRadius(RADIUS);
            circle.setPosition(static_cast<float>(mapped.x - RADIUS), static_cast<float>(mapped.y - RADIUS));
            window.draw(circle);
...
private:
  ...
  struct CustomerData {
      CustomerData(std::int64_t id, std::int64_t x, std::int64_t y): id {id}, x {x}, y {y} {}
      std::int64_t id, x, y;
  };

  std::vector<std::uint32_t>              route_index;
  std::vector<std::uint32_t>              customers_id;
  std::vector<CustomerData>               customers;
  std::vector<std::vector<std::uint32_t>> routes;
  ...
  std::int64_t xmax = std::numeric_limits<std::int64_t>::min(),
               xmin = std::numeric_limits<std::int64_t>::max(),
               ymax = std::numeric_limits<std::int64_t>::min(),
               ymin = std::numeric_limits<std::int64_t>::max();
};
